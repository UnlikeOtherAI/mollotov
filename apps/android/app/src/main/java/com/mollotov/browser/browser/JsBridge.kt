...
/**
 * Android JS bridge exposed as `window.MollotovBridge`.
...
class JsBridge(
    ...
    companion object {
        /** Network traffic capture script. Intercepts XHR and fetch, posts via MollotovBridge. */
        const val NETWORK_BRIDGE_SCRIPT = """
(function() {
    if (!window.MollotovBridge) return;
    var _post = function(data) {
        try { window.MollotovBridge.onNetworkEvent(JSON.stringify(data)); } catch(e) {}
    };
