package com.kelpie.browser.nativecore

object NativeCore {
    init {
        System.loadLibrary("kelpie_jni")
    }

    fun bookmarkStoreCreate(): Long = bookmarkStoreCreateNative().also { check(it != 0L) { "Failed to create bookmark store" } }

    fun historyStoreCreate(): Long = historyStoreCreateNative().also { check(it != 0L) { "Failed to create history store" } }

    fun networkTrafficStoreCreate(): Long = networkTrafficStoreCreateNative().also { check(it != 0L) { "Failed to create network traffic store" } }
    ...
    external fun bookmarkStoreRemove(
        handle: Long,
        id: String,
    )
    ...
    external fun bookmarkStoreLoadJson(
        handle: Long,
        json: String?,
    )
    ...
    external fun historyStoreRecord(
        handle: Long,
        url: String,
        title: String,
    )
    ...
    external fun historyStoreUpdateLatestTitle(
        handle: Long,
        url: String,
        title: String,
    )
    ...
    external fun networkTrafficStoreAppendJson(
        handle: Long,
        entryJson: String,
    ): Boolean
    ...
    external fun networkTrafficStoreSelect(
        handle: Long,
        index: Int,
    ): Boolean
    ...
    external fun networkTrafficStoreLoadJson(
        handle: Long,
        json: String?,
    )
}
