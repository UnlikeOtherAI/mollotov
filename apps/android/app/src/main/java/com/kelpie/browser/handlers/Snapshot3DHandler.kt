...
class Snapshot3DHandler(
    ...
    private suspend fun setMode(body: Map<String, Any?>): Map<String, Any?> {
        if (!context.isIn3DInspector) {
            return com.kelpie.browser.network.errorResponse("NOT_ACTIVE", "3D inspector is not active")
        }
        val requested = (body["mode"] as? String ?: "rotate").lowercase()
        if (requested != "rotate" && requested != "scroll") {
            return com.kelpie.browser.network.errorResponse("INVALID_MODE", "mode must be 'rotate' or 'scroll'")
        }
        return try {
            ...
            com.kelpie.browser.network.successResponse(mapOf("mode" to normalized))
        } catch (error: Exception) {
            com.kelpie.browser.network.errorResponse("JS_ERROR", error.localizedMessage ?: "JavaScript error")
        }
    }
    ...
    private suspend fun zoom(body: Map<String, Any?>): Map<String, Any?> {
        ...
        val delta: Double = when {
            ...
            body["direction"] is String -> when ((body["direction"] as String).lowercase()) {
                "in" -> 0.1
                "out" -> -0.1
        ...
        return try {
            ...
            com.kelpie.browser.network.successResponse(mapOf("delta" to delta))
        } catch (error: Exception) {
            com.kelpie.browser.network.errorResponse("JS_ERROR", error.localizedMessage ?: "JavaScript error")
        }
    }
