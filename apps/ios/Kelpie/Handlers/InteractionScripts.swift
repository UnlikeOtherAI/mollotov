...
func annotationElementsScript() -> String {
    """
    (function() {
...
func selectorActivationScript(_ selector: String) -> String {
    let resolver = """
    (function() {
...
func annotationActivationScript(index: Int) -> String {
    let resolver = "(function() { return (\(annotationElementsScript())).find(function(item) { return item.index === \(index); }); })()"
    return activationScript(resolver: resolver, annotationIndex: index, usingAnnotation: true)
}

func fillElementScript(selector: String, value: String) -> String {
    """
    (function() {
        \(elementSelectorBuilderScript())
...
func fillAnnotationScript(index: Int, value: String) -> String {
    """
    (function() {
        \(elementSelectorBuilderScript())
        \(interactionHelpersScript())
...
func interactionHelpersScript() -> String {
    interactionHelpersScriptBody
}

private let interactionHelpersScriptBody = """
...
private func activationScript(
    resolver: String,
    selector: String? = nil,
    annotationIndex: Int? = nil,
    usingAnnotation: Bool = false
) -> String {
    let selectorLiteral = selector.map { "'\(JSEscape.string($0))'" } ?? "null"
    let annotationLiteral = annotationIndex.map(String.init) ?? "null"
    let activationRequestScript = """
    ...
    (function() {
        \(elementSelectorBuilderScript())
        \(interactionHelpersScript())
        \(activationRequestScript)
        \(activationElementLookup(resolver: resolver, usingAnnotation: usingAnnotation))
        \(activationVisibilityAndFocusScript)
        \(activationEventDispatchScript)
        \(activationSuccessPayloadScript)
    })()
    """
}

private func activationElementLookup(resolver: String, usingAnnotation: Bool) -> String {
    if usingAnnotation {
        return """
...
private let activationEventDispatchScript = """
...
private let activationVisibilityAndFocusScript = """
...
private let activationSuccessPayloadScript = """
return {
