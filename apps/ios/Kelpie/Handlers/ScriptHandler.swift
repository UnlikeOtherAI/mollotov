...
struct ScriptHandler {
    ...
    private func waitForNavigation(_ body: [String: Any]) async -> [String: Any] {
        ...
        let start = Date()
        var observedLoading = context.webView?.isLoading == true

        for _ in 0..<(max(timeout / 100, 1)) {
            if playbackState.isAbortRequested() {
                return abortResponse()
            }
            let isLoading = context.webView?.isLoading == true
            if isLoading {
                observedLoading = true
            }
            if observedLoading, !isLoading {
                return successResponse([
                    "url": context.webView?.url?.absoluteString ?? "",
                    "title": context.webView?.title ?? "",
