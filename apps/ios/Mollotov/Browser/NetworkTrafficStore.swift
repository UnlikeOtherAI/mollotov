...
final class NetworkTrafficStore: ObservableObject {
    ...
    private let handle: MollotovNetworkTrafficStoreRef
    private var isSyncingSelectedIndex = false

    private init() {
        guard let handle = mollotov_network_traffic_store_create() else {
            fatalError("Failed to create Mollotov network traffic store")
        }
    ...
    deinit {
        mollotov_network_traffic_store_destroy(handle)
    }
    ...
    func append(_ entry: TrafficEntry) {
        ...
        let appended = json.withCString { jsonPointer in
            mollotov_network_traffic_store_append_json(handle, jsonPointer)
        }
    ...
    func clear() {
        mollotov_network_traffic_store_clear(handle)
        persistAndRefresh()
    }
    ...
    private func bootstrapFromPersistence() {
        ...
        json.withCString { jsonPointer in
            mollotov_network_traffic_store_load_json(handle, jsonPointer)
        }
    ...
    private func applySelectedIndex() {
        if let selectedIndex {
            let selected = mollotov_network_traffic_store_select(handle, Int32(selectedIndex))
            if selected == 1 {
                syncSelectedIndexFromNative()
                return
            }
    ...
    private func syncSelectedIndexFromNative() {
        ...
        let nativeIndex = mollotov_network_traffic_store_selected_index(handle)
        selectedIndex = nativeIndex >= 0 ? Int(nativeIndex) : nil
        isSyncingSelectedIndex = false
    }

    private func copyNativeEntriesJSON() -> String? {
        copyNativeString { mollotov_network_traffic_store_to_json(handle) }
    }
