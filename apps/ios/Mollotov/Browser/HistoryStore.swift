...
final class HistoryStore: ObservableObject {
    ...
    private let handle: MollotovHistoryStoreRef

    private init() {
        guard let handle = mollotov_history_store_create() else {
            fatalError("Failed to create Mollotov history store")
        }
    ...
    deinit {
        mollotov_history_store_destroy(handle)
    }

    func record(url: String, title: String) {
        url.withCString { urlPointer in
            title.withCString { titlePointer in
                mollotov_history_store_record(handle, urlPointer, titlePointer)
            }
        }
        persistAndRefresh()
    }
    ...
    func clear() {
        mollotov_history_store_clear(handle)
        persistAndRefresh()
    }
    ...
    func updateLatestTitle(for url: String, title: String) {
        url.withCString { urlPointer in
            title.withCString { titlePointer in
                mollotov_history_store_update_latest_title(handle, urlPointer, titlePointer)
            }
        }
        persistAndRefresh()
    }
    ...
    private func bootstrapFromPersistence() {
        ...
        json.withCString { jsonPointer in
            mollotov_history_store_load_json(handle, jsonPointer)
        }
    ...
    private func copyNativeJSON() -> String? {
        guard let pointer = mollotov_history_store_to_json(handle) else {
            return nil
        }
