...
/// State + storage for the pairing/auth system on macOS.
///
/// Persistent state (token hashes only) is written atomically to
/// `<AppSupport>/Kelpie/pairings.json` with mode 0600. No Keychain — per
/// project policy macOS must not touch the Keychain.
...
final class PairingStore: @unchecked Sendable {
    ...
    static func defaultStoreURL() -> URL? {
        ...
        if !fm.fileExists(atPath: dir.path) {
            ...
            // Tighten permissions in case the directory pre-existed with broader bits.
    ...
    /// on the 64-char hex digest below.
    ...
    /// Visible pending requests, oldest first.
