...
extension Notification.Name {
    static let showWelcomeCard = Notification.Name("com.mollotov.browser.macos.show-welcome-card")
}
...
private struct BrowserCommands: Commands {
    ...
    private func openNewWindow() {
        MollotovApp.openNewWindow()
    }

    private func openHelpURL(_ value: String) {
        MollotovApp.openHelpURL(value)
    }

    private func toggleFullScreenForActiveWindow() {
        MollotovApp.toggleFullScreenForActiveWindow()
    }
}
...
struct MollotovApp: App {
    ...
    private func startServices() {
        ...
        if Self.canBind(port: 8421) {
            ...
            print("[MollotovApp] Skipping AppReveal start because port 8421 is already in use")
        }
        #endif
    }
