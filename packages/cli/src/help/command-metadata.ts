...
export const commandMetadata: Record<string, CommandHelp> = {
  ...
  browser: { purpose: "Manage local macOS browser aliases", when: "You want to register, launch, inspect, or remove CLI-managed macOS browser instances on the local machine", explanation: "Provides local browser management commands backed by ~/.mollotov. These commands are separate from mDNS discovery and are intended for launching and reusing named local Mollotov macOS instances across multiple terminal sessions.", related: ["browser register", "browser launch", "browser list", "browser inspect", "browser remove"] },
  "browser register": { purpose: "Register a named local macOS browser alias", when: "You want a stable identifier for a local Mollotov.app instance before launching it", explanation: "Stores a named browser alias in ~/.mollotov, optionally with an explicit Mollotov.app path. Registration does not launch the app or reserve a port.", related: ["browser launch", "browser inspect", "browser remove"] },
  "browser launch": { purpose: "Launch a named local macOS browser instance", when: "You need a fresh Mollotov.app process for a specific local alias", explanation: "Resolves the alias, verifies Mollotov.app is installed, chooses a safe HTTP port if one is not provided, launches a new macOS app instance, and records the live port in ~/.mollotov so later CLI commands can target the alias directly.", errors: ["BROWSER_NOT_REGISTERED", "APP_NOT_INSTALLED", "BROWSER_LAUNCH_FAILED"], related: ["browser register", "browser list", "browser inspect"] },
  "browser list": { purpose: "List registered local macOS browser aliases", when: "You need to see which local aliases exist and whether they are currently reachable", explanation: "Lists all browser aliases stored in ~/.mollotov, including any saved runtime port and whether the local HTTP server is currently reachable.", related: ["browser inspect", "browser launch"] },
  ...
  "browser remove": { purpose: "Remove a local macOS browser alias", when: "A saved alias is no longer needed", explanation: "Deletes the alias and saved runtime state from ~/.mollotov. It does not terminate any running app process.", related: ["browser list", "browser register"] },
