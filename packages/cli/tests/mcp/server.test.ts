...
describe("createMcpServer", () => {
  ...
  it("registers 137 tools total (114 browser + 23 CLI)", () => {
    expect(browserTools).toHaveLength(114);
    expect(cliTools).toHaveLength(23);
    expect(browserTools.length + cliTools.length).toBe(137);
  });
});
